...
class GenerationService {
    ...
    getImageKitConfig() {
        return {
            ...
            urlEndpoint: process.env.IMAGEKIT_URL_ENDPOINT || 'https://ik.imagekit.io/jomobit'
        };
    }
    ...
    async processGenerationJob(jobId) {
        ...
        try {
            ...
            if (!job) {
                throw new GenerationError('Generation job not found', 'JOB_NOT_FOUND', { jobId });
            }
    ...
    async generateImage(job, prompt) {
        ...
        try {
            ...
            const diffusionProvider = providerFactory.createDiffusionProvider(job.aiProvider.diffusion);
    ...
    async handleGenerationFailure(jobId, error) {
        ...
        try {
            ...
            if (!job) {
                throw new GenerationError('Job not found for failure handling', 'JOB_NOT_FOUND', { jobId });
            }
            ...
            try {
                ...
                if (creditError.name === 'CreditOperationError' &&
    ...
    async validateGenerationRequest(userId, profileId, templateId, aiProvider) {
        ...
        if (!availableDiffusion.includes(aiProvider.diffusion)) {
            throw new GenerationValidationError(
                `Invalid diffusion provider: ${aiProvider.diffusion}. Available: ${availableDiffusion.join(', ')}`,
    ...
    async cancelGenerationJob(jobId, userId, reason = 'User cancelled') {
        ...
        try {
            ...
            if (!job) {
                throw new GenerationError('Job not found or access denied', 'JOB_NOT_FOUND', { jobId });
            }
